...
const phraseWords = ["A", "Piece", "of", "Cake"];
const colors = [
  "#FF5733",
  "#DAF7A6",
  "#FFC300",
...
const init = () => {
  ...
  scrambleBtn.addEventListener("click", () =>
    scramble(Math.floor(Math.random() * 100))
  );
...
const getNeighbour = (pos) => {
  ...
  for (let i = -1; i < 2; i++) {
    for (let j = -1; j < 2; j++) {
      ...
      if (game[pos.y + i][pos.x + j]) n.push({ x: pos.x + j, y: pos.y + i });
    }
  }
  return n;
};
...
const lookEmptyPos = (pos) => {
  for (let i = -1; i < 2; i++) {
    for (let j = -1; j < 2; j++) {
      if (checkInvalid(pos.x, pos.y, i, j)) continue;
      if (!game[pos.y + i][pos.x + j]) return { x: pos.x + j, y: pos.y + i };
    }
  }
  return null;
};
